...
var query5 =
    from aluno in uni.Alunos
    from turma in uni.Turmas
    where aluno.Matriculas.Contains(turma.Id)
    join prof in uni.Professores
    on turma.ProfessorId equals prof.Id
    group prof by aluno.Nome into g
    select new {
        Aluno = g.Key,
        Custo = 300 + (
            from prof in g
            join turma in uni.Turmas
            on prof.Id equals turma.ProfessorId
            from aluno in uni.Alunos
            where aluno.Matriculas.Contains(turma.Id)
            group aluno by new {prof.Id, prof.Salario} into profSalario
            let alunosCount = profSalario.Count()
            select profSalario.Key.Salario / alunosCount
        ).Sum()
    };
...
foreach (var aluno in query5)
{
    ...
    // """);
    WriteLine($"""
    Nome: {aluno.Aluno} | Custo: R$ {Math.Round(aluno.Custo, 2)}
    -----------------------------------
    """);
}
